...
__global__ void jhcunn_LookupTable_accGradParametersKernelByFeature(
  float *input,
  float *gradOutput,
  float *gradWeight,
  float *scale,
  ...
  for (int i = laneId; i < numel; i += WARP_SIZE) {
    ...
    float update = gradOutput[i*stride + featureDim] * scale[i / batchSize];
...
__global__ void jhcunn_LookupTable_accGradParametersKernel(
  float *input,
  float *indices,
  float *gradOutput,
  float *gradWeight,
