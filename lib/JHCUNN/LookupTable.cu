...
__global__ void cunn_LookupTable_accGradParametersKernelByFeature(
  ...
  int paddingValue) {
    ...
    for (int i = laneId; i < numel; i += WARP_SIZE) {
        ...
        // const int batchIndex = i / (int)batchSize;
        // printf("i = %d weightIndex = %d batchIndex = %d batchSize = %ld\n", i, weightIndex, batchIndex, batchSize);
        float update = gradOutput[i*stride + featureDim] * scale[i];
...
__global__ void cunn_LookupTable_accGradParametersKernel(
  ...
  if (idx < numel
    ...
    do {
      ...
      const float scale = count ? defaultScale[idx] / count[idx] : defaultScale[idx];
  ...
  THCudaTensor *scale)
{
  ...
  if(numel != THCudaTensor_size(state, scale, 0)) {
      THError("size mismatch between input numel and scale dim 0");
  }
